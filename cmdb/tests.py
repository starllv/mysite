...
class QuestionIndexViewTests(TestCase):
	def text_no_questions(self):
		response = self.client.get(reverse('cmdb:index'))
		self.assertEqual(response.status_code,200)
		self.assertContains(response,"No cmdb are avilable.")
	...
	def test_future_question(self):
		...
		response = self.client.get(reverse('cmdb:index'))
		self.assertContains(response,"No cmdb are available.")
	...
	def test_future_question_and_past_question(self):
		...
		create_question(question_text="未来的问题",days=30)
		response = self.client.get(reverse('cmdb:index'))
		self.assertQuerysetEqual(response.context['latest_question_list'],['<Question:过去的问题>'，'<Question:未来的问题>'])
...
class QuestionDetailViewTests(TestCase):
	def test_future_question(self):
		...
		url = reverse('cmdb:detail',args=(future_question.id))
		response = self.client.get(url)
		self.assertEqual(response.status_code,404)
		
	def test_past_question(self):
		past_question = create_question(question_text="过去的问题",days=-5)
		url = reverse('cmdb:detail',args=(past_question.id))
		response = slef.clinet.get(url)
